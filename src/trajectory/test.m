...
iou_threshold = 0.5;
anno = read_anno(anno_root, video_root, video_name);
...
for f = 1:size(anno, 1)
    ...
    for o = 1:size(anno, 2)
        ...
        x_min = object_box(1);
        x_max = min(x_min + object_box(3), h);
        y_min = object_box(2);
        y_max = min(y_min + object_box(4), w);
        box_flow_x = flow(x_min:x_max, y_min:y_max, 1);
        box_flow_y = flow(x_min:x_max, y_min:y_max, 2);
