...
if ~exist(fullfile(output_root, 'scene'), 'dir')
    mkdir(fullfile(output_root, 'scene'));
end
if nargin==2
    ...
    for i = 3:length(pspnet_scene_videos)
        ...
        save_scene_path = fullfile(output_root, 'scene',pspnet_scene_video.name);
...
if nargin==3
        save_scene_path = fullfile(output_root, 'scene', video_name);
