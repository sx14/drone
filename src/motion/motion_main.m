...
if ~exist(fullfile(output_root, 'motion'), 'dir')
    mkdir(fullfile(output_root, 'motion'));
end
if nargin==3
    flow_videos = dir(fullfile(flow_root,'*'));
    for i = 3:length(flow_videos)
        flow_video = flow_videos(i);
        save_motion_path = fullfile(output_root,'motion',flow_video.name);
...
if nargin==4
   save_motion_path = fullfile(output_root,'motion',video_name);
